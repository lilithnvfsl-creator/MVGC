...
default_cfgs = {
    'mvgc': _cfg(crop_pct=0.9, mean=IMAGENET_DEFAULT_MEAN, std=IMAGENET_DEFAULT_STD)
}
...
class DepthWiseSeparable(nn.Module):
    def __init__(self, in_dim, kernel, e=4):
        ...
        self.dw = nn.Conv2d(in_dim * e, in_dim * e, kernel_size=kernel, stride=1, padding=1,
                            groups=in_dim * e)  # kernel size = 3
...
class MVGC(torch.nn.Module):
    def __init__(self, blocks, channels, dropout=0., drop_path=0., emb_dims=512, rep=False,
        ...
        self.init_weights()
        # self = torch.nn.BatchNorm2d.convert_sync_batchnorm(self)
